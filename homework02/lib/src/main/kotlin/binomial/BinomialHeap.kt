...
class BinomialHeap<T : Comparable<T>> private constructor(private val trees: FList<BinomialTree<T>?>) :
    ...
    private fun getTreeOrderNS(tree: BinomialTree<T>?, default: Int): Int = tree?.order ?: default

    private fun plusImpl(
        first: FList<BinomialTree<T>?>,
        second: FList<BinomialTree<T>?>,
        overflow: BinomialTree<T>?
        ...
        if (overflow == null) {
            ...
            firstMergeVal = if (firstElemOrd == expectedTreeOrder) overflow.let { firstElem!! + (it) } else overflow
        }
    ...
    override fun plus(other: BinomialHeap<T>): BinomialHeap<T> =
        BinomialHeap(plusImpl(trees, other.trees, null).reverse())
    ...
    private fun withoutTree(tree: BinomialTree<T>?): FList<BinomialTree<T>?> = trees.filter { e -> e != tree }
    private fun getTreeWithMin(min: T): BinomialTree<T>? {
        val resultTrees = trees.filter { e -> e!!.value == min }
        if (resultTrees.size > 0) {
            return (resultTrees as FList.Cons<BinomialTree<T>>).head
        }
        return null
    }
    ...
    fun drop(): BinomialHeap<T> {
        val treeWithMin = getTreeWithMin(top()) ?: throw NoSuchElementException("Binomial heap is empty!")
        return BinomialHeap(withoutTree(treeWithMin)) + BinomialHeap(treeWithMin.children.reverse())
    }
}
